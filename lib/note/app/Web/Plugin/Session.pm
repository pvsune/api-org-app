package note::app::Web::Plugin::Session;
use strict;
use warnings;
use utf8;
...
my $cipher = Crypt::CBC->new({
    key => 'Q5fAWTftGul6WLevMxFrFt7ws_p7wqsY',
    cipher => 'Rijndael',
});
sub _session {
    my $self = shift;
    ...
    if (!exists $self->{session}) {
        $self->{session} = HTTP::Session2::ClientStore2->new(
            env => $self->req->env,
            secret => 'SQwFlt3eyR39mAQWg62G-ZHtbP5RZQdT',
            cipher => $cipher,
        );
    }
    return $self->{session};
...
This module manages session for note::app.
